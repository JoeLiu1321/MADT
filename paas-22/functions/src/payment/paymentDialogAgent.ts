...
import { ASSISTANT, sessionServiceUrl } from "./paymentConfig"
import { DialogMessage, ReplyMessage } from "./paymentModel"
...
export const dialogAgent = async (request:any, dialogMessage: DialogMessage): Promise<any> => {
    const sessionId = await axios.post(sessionServiceUrl + `payment/sessionId`, { userId: dialogMessage.chatId }).then(result => {
